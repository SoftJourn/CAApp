...
func (setup *BaseSetupImpl) Initialize() error {
	...
	if err := setup.InstallAndInstantiateExampleCC(); err != nil {
		return errors.Wrapf(err, "err create channel (%s) failed: %v", err)
	}
	...
	//alreadyJoined, err := integration.HasPrimaryPeerJoinedChannel(sc, channel)
	//if err != nil {
...
func (setup *BaseSetupImpl) GetDeployPath() string {
	pwd, _ := os.Getwd()
	return path.Join(pwd, "../CAApp/fixtures/testdata")
}
...
func (setup *BaseSetupImpl) InstallAndInstantiateExampleCC() error {
	...
	if setup.ChainCodeID == "" {
		setup.ChainCodeID = integration.GenerateRandomID()
	}
